...
export async function runAutomod(message: Message) {
  ...
  try {
    const parsed: unknown = JSON.parse(config.blockedWords);
    if (Array.isArray(parsed)) blockedWords = parsed.filter((word): word is string => typeof word === "string");
  } catch {
    blockedWords = [];
  }
  ...
  if (!blocked && !invite && !spam) return false;
  await message.delete().catch(() => undefined);
  if (spam && message.member?.moderatable) await message.member.timeout(5 * 60_000, "Nymera automod: message spam").catch(() => undefined);
