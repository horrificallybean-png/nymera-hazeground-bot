...
function pickWinners(userIds: string[], count: number) {
  const pool = [...userIds];
...
export function startGiveawayMonitor(client: Client) {
  const timer = setInterval(async () => {
    try {
      ...
      for (const giveaway of due) await endGiveaway(client, giveaway.id);
    } catch (error) {
...
export function giveawayEmbed(prize: string, endsAt: Date, id?: number) {
