...
const WEEK = 7 * DAY;
const rewardCommand = (name: "daily" | "weekly", cooldown: number, amount: number): Command => ({
  ...
  async execute(i) {
    ...
    if (wait) return void await i.reply({ content: `Return in ${formatDuration(wait)}.`, ephemeral: true });
...
export const economyCommands: Command[] = [
  ...
  rewardCommand("weekly", WEEK, 1500),
  {
    ...
    async execute(i) {
      ...
      if (wait) return void await i.reply({ content: `Rest for ${formatDuration(wait)}.`, ephemeral: true });
  ...
  ...(["deposit", "withdraw"] as const).map((name): Command => ({
  ...
  })),
  {
    ...
    async execute(i) {
      ...
      if (user.bot || user.id === i.user.id) return void await i.reply({ content: "Choose another human member.", ephemeral: true });
      const amount = i.options.getInteger("amount", true);
      try {
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (!item?.enabled || (item.stock !== null && item.stock < quantity)) return void await i.reply({ content: "That item is unavailable.", ephemeral: true });
      ...
      try {
        await prisma.$transaction(async tx => {
          ...
          if (a.wallet < cost) throw new Error("INSUFFICIENT_FUNDS");
          ...
          if (item.stock !== null) await tx.shopItem.update({ where: { id: item.id }, data: { stock: { decrement: quantity } } });
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (!quest) return void await i.reply({ content: "Quest not found. Use `/quests`.", ephemeral: true });
      ...
      if (!progress || progress.progress < quest.target || progress.claimedAt) return void await i.reply({ content: "That quest is incomplete or already claimed.", ephemeral: true });
      await prisma.$transaction([
  ...
  },
  {
    ...
    async execute(i) {
      ...
      } catch { await i.reply({ content: "That change would make the wallet negative.", ephemeral: true }); }
    }
  }
];
