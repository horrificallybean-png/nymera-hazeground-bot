...
const sessions = new Map<string, { word: string; guessed: Set<string>; misses: number; ownerId: string }>();
const sessionKey = (guildId: string, channelId: string) => `${guildId}:${channelId}`;
const checkCooldown = (guildId: string, userId: string) => {
  const key = `${guildId}:${userId}`;
  const now = Date.now();
  const wait = Math.max(0, (cooldowns.get(key) ?? 0) - now);
  if (!wait) cooldowns.set(key, now + 5_000);
  return wait;
};

const allGameCommands: Command[] = [
  {
    ...
    async execute(i) {
      if (checkCooldown(i.guildId!, i.user.id)) return void await i.reply({ content: "The coin is still spinning. Wait a few seconds.", ephemeral: true });
      ...
      } catch { await i.reply({ content: "You do not have enough Spellmarks.", ephemeral: true }); }
    }
  },
  {
    ...
    async execute(i) {
      if (checkCooldown(i.guildId!, i.user.id)) return void await i.reply({ content: "Wait a few seconds before playing again.", ephemeral: true });
  ...
  },
  {
    data: new SlashCommandBuilder().setName("haunted-slots").setDescription("Spin the haunted slots")
      .addIntegerOption(o => o.setName("bet").setDescription("Spellmarks wager").setRequired(true).setMinValue(10).setMaxValue(5000)),
    async execute(i) {
      if (checkCooldown(i.guildId!, i.user.id)) return void await i.reply({ content: "The reels need a moment.", ephemeral: true });
      const bet = i.options.getInteger("bet", true);
      const symbols = ["🌙", "🔮", "🕯️", "💀", "🦇"];
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (i.options.getSubcommand() === "start") {
        if (sessions.has(key)) return void await i.reply({ content: "A hangman game is already active here.", ephemeral: true });
      ...
      if (!session) return void await i.reply({ content: "Start a game first.", ephemeral: true });
      const letter = i.options.getString("letter", true).toLowerCase();
      if (!/^[a-z]$/.test(letter) || session.guessed.has(letter)) return void await i.reply({ content: "Choose a new A–Z letter.", ephemeral: true });
      session.guessed.add(letter);
      if (!session.word.includes(letter)) session.misses++;
  ...
  },
  {
    data: new SlashCommandBuilder().setName("game-stats").setDescription("View a member's game statistics")
      .addStringOption(o => o.setName("view").setDescription("Statistics view").addChoices(
        { name: "Member statistics", value: "member" },
        { name: "Server leaderboard", value: "leaderboard" }
      ))
