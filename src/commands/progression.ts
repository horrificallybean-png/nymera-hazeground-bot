...
async function createReferralCode(guildId: string, userId: string) {
...
export const progressionCommands: Command[] = [
  {
      ...
      if (action === "info") {
        await i.reply({
          embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle("Prestige Through the Veil")
            ...
            .addFields(
              { name: "Current prestige", value: String(account.prestige), inline: true },
              { name: "Current level", value: `${account.level}/${PRESTIGE_LEVEL}`, inline: true },
              { name: "Next reward", value: `${reward} ${currency}`, inline: true }
            )]
        });
        return;
      }
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (i.options.getSubcommand() === "view") {
        ...
        await i.reply({
          embeds: [new EmbedBuilder().setColor(challenge.completedAt ? 0x2ecc71 : 0x6f42c1).setTitle("Weekly Coven Challenge")
            ...
            .addFields(
              { name: "Server progress", value: `${Math.min(challenge.progress, challenge.target)}/${challenge.target}`, inline: true },
              { name: "Your contribution", value: `${contribution?.progress ?? 0}`, inline: true },
              { name: "Reward", value: `${challenge.reward} ${currency}`, inline: true },
              { name: "Top contributors", value: leaders.map((row, n) => `${n + 1}. ${userMention(row.userId)} — ${row.progress}`).join("\n") || "No contributions yet." }
            )
            .setFooter({ text: `Contribute at least ${MIN_CHALLENGE_CONTRIBUTION} eligible messages to claim.` })]
        });
        return;
      }
      if (!challenge.completedAt) return void await i.reply({ content: "The coven challenge is not complete yet.", ephemeral: true });
      ...
      if (contribution.claimedAt) return void await i.reply({ content: "You already claimed this week's reward.", ephemeral: true });
      await prisma.$transaction([
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (!referral || referral.guildId !== i.guildId) return void await i.reply({ content: "That referral code is not valid in this server.", ephemeral: true });
      if (referral.userId === i.user.id) return void await i.reply({ content: "You cannot claim your own referral code.", ephemeral: true });
      ...
      const account = await getAccount(i.guildId!, i.user.id);
      if (account.messages > 50) return void await i.reply({ content: "Referral codes are for newer community members and must be claimed before 50 eligible messages.", ephemeral: true });
      try {
        await prisma.$transaction([
