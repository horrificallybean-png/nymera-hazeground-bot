...
export const utilityCommands: Command[] = [
    ...
    async execute(i) {
      await i.reply({
        embeds: [new EmbedBuilder()
          ...
          .setDescription("**AI:** `/ask`\n**Community:** `/ticket`, `/reaction-role`, `/role-buttons`, `/giveaway-start`, `/giveaway-enter`, `/giveaway-end`\n**Levels:** `/rank`, `/levels`, `/level-settings`, `/level-role`, `/xp-admin`\n**Magic:** `/tarot`, `/daily-draw`, `/moon`, `/planetary-hour`, `/herb`, `/grimoire`, `/magic-search`\n**Economy:** `/balance`, `/daily`, `/weekly`, `/work`, `/shop`, `/buy`, `/inventory`, `/quests`\n**Games:** `/coinflip`, `/bone-dice`, `/haunted-slots`, `/horror-trivia`, `/haunted-hangman`, `/auto-games`\n**Configuration:** `/setup`, `/settings`, `/schedule`, `/mental-health-checkin`, `/mental-health-space`, plus moderation commands.")
        ],
        ephemeral: true
      });
    }
  ...
  },
  {
    ...
    async execute(i) {
      if (!i.guild) return void await i.reply({ content: "Use this in a server.", ephemeral: true });
      await i.reply({ embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle(i.guild.name)
        .addFields(
          ...
          { name: "Created", value: time(i.guild.createdAt, "R"), inline: true },
          { name: "Owner", value: userMention(i.guild.ownerId), inline: true }
        )] });
    }
  },
  {
