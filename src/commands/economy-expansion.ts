...
const bankCapacity = (level: number) => 5_000 * (level + 1);
const bankUpgradeCost = (level: number) => 2_000 * (level + 1);
...
const profileType = (key: string) => {
  if (key.endsWith("_title")) return "title";
  if (key.endsWith("_badge")) return "badge";
  if (key.endsWith("_background")) return "background";
  return null;
};

export const economyExpansionCommands: Command[] = [
  {
    ...
    async execute(i) {
      ...
      if (action === "info") {
        ...
        await i.reply({
          embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle("Mistbound Bank")
            .addFields(
              { name: "Bank tier", value: `${account.bankLevel + 1}/11`, inline: true },
              { name: "Savings", value: `${account.bank}/${capacity} ${currency}`, inline: true },
              { name: "Daily interest", value: `${rate}%`, inline: true },
              { name: "Next upgrade", value: account.bankLevel >= 10 ? "Maximum tier" : `${bankUpgradeCost(account.bankLevel)} ${currency}`, inline: true },
              { name: "Interest ready", value: wait ? `In ${formatDuration(wait)}` : "Now", inline: true }
            )]
        });
        return;
      }
      if (action === "upgrade") {
        if (account.bankLevel >= 10) return void await i.reply({ content: "Your bank is already at its maximum tier.", ephemeral: true });
        const cost = bankUpgradeCost(account.bankLevel);
        if (account.wallet < cost) return void await i.reply({ content: `You need ${cost} ${currency} in your wallet.`, ephemeral: true });
      ...
      const wait = remaining(account.bankInterestAt, DAY);
      if (wait) return void await i.reply({ content: `Interest returns in ${formatDuration(wait)}.`, ephemeral: true });
      if (account.bank < 100) return void await i.reply({ content: `Keep at least 100 ${currency} in the bank to earn interest.`, ephemeral: true });
      if (account.bank >= capacity) return void await i.reply({ content: "Your bank is full. Withdraw some savings or upgrade it before claiming interest.", ephemeral: true });
  ...
  },
  {
    ...
    async execute(i) {
      ...
      if (action === "view") {
        ...
        await i.reply({
          embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle(`${user.username}'s Hazeground Profile`)
            ...
            .addFields(
              { name: "Title", value: account.equippedTitle ? profileLabels[account.equippedTitle] ?? account.equippedTitle : "None", inline: true },
              { name: "Badge", value: account.equippedBadge ? profileLabels[account.equippedBadge] ?? account.equippedBadge : "None", inline: true },
              { name: "Level", value: String(account.level), inline: true },
              { name: "Wealth", value: `${account.wallet + account.bank} ${currency}`, inline: true },
              { name: "Collection", value: `${collection} unique items`, inline: true }
            )]
        });
        return;
      }
      if (action === "equip") {
        ...
        if (!type) return void await i.reply({ content: "That item is not an equipable title, badge, or background.", ephemeral: true });
        const owned = await prisma.inventoryItem.findFirst({
          where: { guildId: i.guildId!, userId: i.user.id, quantity: { gt: 0 }, item: { key } }
        });
        if (!owned) return void await i.reply({ content: "You do not own that item. Check `/shop` and `/inventory`.", ephemeral: true });
