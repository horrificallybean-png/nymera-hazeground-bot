...
async function sendAiModerationReview(message, trigger) {
    ...
    if (!config?.aiModerationEnabled || !config.aiReviewChannelId)
        return;
    ...
    await channel.send({ embeds: [new EmbedBuilder()
                ...
                .addFields({ name: "Member", value: `${message.author.tag} (${message.author.id})` }, { name: "Message", value: message.content.slice(0, 1000) || "*No text*" }, { name: "Human review required", value: "Nymera took no AI moderation action. Staff must review context and decide." })
...
export function registerEvents(client) {
    client.once(Events.ClientReady, async (ready) => {
        ...
        void (async () => {
            ...
            if (env.DISCORD_GUILD_ID) {
                ...
                if (!guild)
                    throw new Error(`Configured guild ${env.DISCORD_GUILD_ID} is not available to Nymera`);
                if (!configuredGuild) {
                    logger.warn({
    ...
    client.on(Events.InteractionCreate, async (interaction) => {
        if (interaction.isButton() && interaction.customId.startsWith("rolebutton:")) {
            try {
                if (!interaction.guildId || !interaction.guild || !interaction.message)
                    return;
    ...
    client.on(Events.GuildMemberAdd, async (member) => {
        ...
        if (c.autoRoleId)
            await member.roles.add(c.autoRoleId, "Nymera autorole").catch(() => undefined);
        if (c.welcomeChannelId) {
            ...
            if (channel && "send" in channel) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`${member}, you've entered a realm of **magic, mystery, and mayhem**.\n\n` +
    ...
    client.on(Events.GuildMemberRemove, async (member) => {
        ...
        if (c.goodbyeChannelId) {
            ...
            if (channel && "send" in channel)
                await channel.send(`**${member.user.tag}** has departed into the mist.`);
        }
    ...
    client.on(Events.MessageReactionAdd, async (reaction, user) => {
        ...
        try {
            ...
            if (!config.starboardChannelId || stars < config.starboardThreshold)
                return;
            const target = await guild.channels.fetch(config.starboardChannelId).catch(() => null);
            if (!target || !("send" in target) || !reaction.message.author)
                return;
    ...
    client.on(Events.MessageCreate, async (message) => {
        try {
            ...
            if (await handleContinuousGameMessage(message))
                return;
                    ...
                    if (result.leveledUp && message.member) {
                        ...
                        if (config.levelUpEnabled) {
                            ...
                            if (target && "send" in target) {
                                ...
                                await target.send(`✨ ${message.author}, you reached **level ${result.level}** and earned **${result.reward} Spellmarks**!${roles}`).catch(() => undefined);
                            }
                        }
                    }
                }
            ...
            if (!mentioned && !eligibleAutoReply)
                return;
            if (!c.aiEnabled)
                return;
            if (eligibleAutoReply)
                autoReplyCooldowns.set(message.channel.id, Date.now());
            const prompt = message.content.replace(new RegExp(`<@!?${client.user.id}>`, "g"), "").trim();
            if (!prompt)
                return void await message.reply("You called through the mist?");
