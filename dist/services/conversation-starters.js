...
export async function postConversationStarter(client, guildId) {
    if (postingGuilds.has(guildId))
        return { ok: false, reason: "Nymera is already preparing a conversation starter." };
    const config = await prisma.guildConfig.findUnique({ where: { guildId } });
    if (!config?.aiEnabled)
        return { ok: false, reason: "Nymera's AI is disabled. Enable it with `/setup ai_enabled:True`." };
    if (!config.aiConversationStarterEnabled)
        return { ok: false, reason: "Conversation starters are disabled. Enable `start_conversations` with `/ai-settings`." };
    if (!config.aiConversationChannelId)
        return { ok: false, reason: "No conversation channel is configured. Set `conversation_channel` with `/ai-settings`." };
...
async function postDueStarters(client) {
    ...
    for (const config of configs) {
        ...
        if (!result.ok)
            logger.warn({ guildId: config.guildId, reason: result.reason }, "Scheduled conversation starter did not post");
    }
}
export function startConversationStarterMonitor(client) {
    if (starterTimer)
