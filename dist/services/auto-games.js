...
async function reward(guildId, userId, amount, game) {
...
async function hostChoice(channel, activity, config) {
    ...
    collector.on("collect", async (interaction) => {
        if (!interaction.customId.startsWith(`autogame:${sessionId}:`))
            return;
        if (answered.has(interaction.user.id))
            return void await interaction.reply({ content: "You already answered.", ephemeral: true });
        answered.add(interaction.user.id);
        const won = Number(interaction.customId.split(":")[2]) === generated.answer;
        try {
            if (won)
                await reward(config.guildId, interaction.user.id, 100, activity.game);
    ...
    collector.on("end", () => {
        const disabled = new ActionRowBuilder().addComponents(generated.choices.map((choice, index) => new ButtonBuilder().setCustomId(`ended:${sessionId}:${index}`).setLabel(`${index + 1}. ${choice}`)
...
async function hostTreasure(channel, activity, guildId, roleIds, seconds) {
    ...
    const collector = message.createMessageComponentCollector({ componentType: ComponentType.Button, time: seconds * 1000 });
    collector.on("collect", interaction => {
        if (tried.has(interaction.user.id))
            return void interaction.reply({ content: "You already chose a chest.", ephemeral: true });
        tried.add(interaction.user.id);
        const won = Number(interaction.customId.split(":")[2]) === winningChest;
        if (!won)
            return void interaction.reply({ content: "Only dust waited inside.", ephemeral: true });
        void interaction.deferUpdate();
...
async function hostCounting(channel, activity, guildId, roleIds, seconds) {
    ...
    collector.on("collect", message => {
        if (message.author.id === lastUser || message.content.trim() !== String(next))
            return;
...
async function hostWordChain(channel, activity, guildId, roleIds, seconds) {
    ...
    collector.on("collect", (message) => {
        ...
        if (!/^[a-z]{2,20}$/.test(word) || used.has(word) || message.author.id === lastUser || word[0] !== previous.at(-1))
            return;
        used.add(word);
        previous = word;
        lastUser = message.author.id;
    ...
    collector.on("end", (_collected, reason) => {
        if (reason !== "complete")
            void channel.send(`The chain ended after **${accepted}** accepted word(s). Last word: **${previous}**.`);
    });
}
export async function launchAutoGame(client, guildId) {
...
export function startAutoGameMonitor(client) {
    const timer = setInterval(async () => {
        try {
            ...
            for (const config of configs) {
                ...
                if (now >= dueAt)
                    await launchAutoGame(client, config.guildId);
            }
        }
        catch (error) {
