...
function enqueue(channelId, work) {
...
function parseUsedWords(value) {
    try {
        const parsed = JSON.parse(value);
        return Array.isArray(parsed) ? parsed.filter(word => typeof word === "string") : ["moon"];
    }
    catch {
        return ["moon"];
    }
}
export async function postContinuousGameInstructions(counting, wordChain) {
...
async function handleCounting(message, guildId) {
    ...
    if (submitted % 100 === 0)
        await channel.send(`✨ The coven reached **${submitted}**! The endless count continues with **${submitted + 1}**.`);
}
async function handleWordChain(message, guildId) {
    const channel = message.channel;
    const state = await prisma.continuousGameConfig.findUnique({ where: { guildId } });
    if (!state)
        return;
    ...
    await message.react("✅").catch(() => undefined);
    if (nextUsed.length % 50 === 0)
        await channel.send(`⛓️ The chain reached **${nextUsed.length} words**! Continue with **${word.at(-1)?.toUpperCase()}**.`);
}
export async function handleContinuousGameMessage(message) {
    ...
    if (!counting && !wordChain)
        return false;
    await enqueue(message.channelId, async () => {
        try {
            if (counting)
                await handleCounting(message, message.guildId);
            else
                await handleWordChain(message, message.guildId);
        }
        catch (error) {
