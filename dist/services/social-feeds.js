...
function attribute(block, element, name) {
...
export async function fetchSocialFeed(feedUrl) {
    ...
    if (!items.length)
        throw new Error("No RSS or Atom posts were found at that URL");
    return items;
}
function postEmbed(platform, item) {
...
export async function checkSocialFeed(client, feedId, force = false) {
    const feed = await prisma.socialFeed.findUnique({ where: { id: feedId } });
    if (!feed?.enabled)
        return { ok: false, reason: "That automatic feed is disabled." };
    ...
    if (!force && newest.id === feed.lastItemId)
        return { ok: true, announced: false };
    const channel = await client.channels.fetch(feed.channelId).catch(() => null);
    if (!channel || !("send" in channel))
        throw new Error("Nymera cannot access the configured social-alert channel");
...
export function startSocialFeedMonitor(client) {
    if (monitor)
