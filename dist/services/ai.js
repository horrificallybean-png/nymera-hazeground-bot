...
function isTooSimilar(candidate, recent) {
...
export async function generateAutoGameRound(topic, fallback, recentQuestions = []) {
    if (!client) {
...
Create a genuinely different question with a different answer concept. Variety attempt: ${attempt + 1}.`,
            ...
            if (!generated)
                throw new Error("AI did not return a completed game");
            if (isTooSimilar(generated.question, recentQuestions))
                continue;
...
Variety attempt: ${attempt + 1}.`,
            ...
            const generated = response.output_parsed;
            if (!generated)
                throw new Error("AI did not return a completed activity");
            if (input.type === "text" && !generated.answers?.length)
                throw new Error("AI text activity omitted answers");
            if (input.type === "poll" && !generated.choices?.length)
                throw new Error("AI poll omitted choices");
            if (input.type === "wordchain" && !generated.startWord)
                throw new Error("AI word chain omitted startWord");
            if (isTooSimilar(generated.question, input.recentQuestions ?? []))
                continue;
            autoGameAiStatus = { result: "success", attemptedAt: new Date() };
            return generated;
        }
...
export async function generateDynamicScheduledContent(template, fallback, recentContent = []) {
    if (!client || !/\{\{(?:daily_|daily_tarot|herb_lore|moon_phase)/.test(template))
        return fallback;
...
Variety attempt: ${attempt + 1}.`,
            ...
            if (!output || isTooSimilar(output, recentContent))
                continue;
...
export async function generateConversationStarter(fallback, recentContent = []) {
    if (!client)
        return fallback;
    try {
        for (let attempt = 0; attempt < 2; attempt++) {
...
export async function askNymera(input) {
    if (!client)
        return "Nymera's AI is not configured. An administrator must set `OPENAI_API_KEY` and restart the bot.";
    const config = await ensureGuild(input.guildId);
    if (!config.aiEnabled)
        return "Nymera's AI replies are disabled in this server.";
...
export function looksReviewable(content) {
...
export async function createModerationSuggestion(content, trigger) {
    if (!client)
        return null;
    try {
        const response = await client.responses.parse({
