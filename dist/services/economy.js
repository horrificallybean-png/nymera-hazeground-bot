...
export const accountKey = (guildId, userId) => ({
...
export function remaining(last, cooldownMs) {
    if (!last)
        return 0;
...
export async function changeWallet(guildId, userId, amount, type, note) {
...
export const levelForXp = (xp) => Math.floor(Math.sqrt(xp / 100));
export const periodKey = (key, date = new Date()) => {
...
export async function advanceQuestProgress(guildId, userId, kind, amount = 1) {
...
export async function advanceCovenChallenge(guildId, userId, amount = 1) {
...
export async function recordMessage(guildId, userId) {
...
export async function applyLevelRewards(guild, member, level) {
...
export async function seedGuildEconomy(guildId) {
    const items = [
