...
async function deliverScheduledPost(client, postId) {
    ...
    try {
        ...
        const content = await generateDynamicScheduledContent(post.content, fallback, history.map(entry => entry.content));
...
async function synchronizeScheduledTasks(client) {
...
export async function runScheduledPostNow(client, guildId, postId) {
    const post = await prisma.scheduledPost.findFirst({ where: { id: postId, guildId } });
    if (!post)
        return { ok: false, reason: "That scheduled post was not found in this server." };
