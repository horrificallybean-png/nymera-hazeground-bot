...
export async function getTwitchStream(login) {
    ...
    const payload = await response.json();
    if (!response.ok)
        throw new Error(payload.message || `Twitch API returned HTTP ${response.status}`);
    return payload.data?.[0] ?? null;
}
function liveEmbed(stream) {
    ...
    return new EmbedBuilder()
        ...
        .addFields({ name: "Playing", value: stream.game_name || "No category", inline: true }, { name: "Viewers", value: stream.viewer_count.toLocaleString(), inline: true }, { name: "Watch now", value: `[Open the live stream](${streamUrl})` })
        .setImage(thumbnail)
...
export async function checkTwitchAlert(client, guildId, force = false) {
    const config = await prisma.twitchAlertConfig.findUnique({ where: { guildId } });
    if (!config?.enabled)
        return { ok: false, reason: "Twitch live alerts are not enabled." };
    ...
    if (!stream)
        return { ok: true, live: false, reason: `${config.twitchLogin} is currently offline.` };
    if (!force && config.lastStreamId === stream.id)
        return { ok: true, live: true, announced: false };
    const channel = await client.channels.fetch(config.channelId).catch(() => null);
    if (!channel || !("send" in channel))
        throw new Error("Nymera cannot access the configured Twitch-alert channel");
...
export function startTwitchAlertMonitor(client) {
    if (monitor)
