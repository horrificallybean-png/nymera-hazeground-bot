        ...
        async execute(i) {
            ...
            if (sub === "create-themed") {
                ...
                for (const themed of themedLevelRoles) {
                    let role = i.guild.roles.cache.find(candidate => candidate.name.toLowerCase() === themed.name.toLowerCase());
                    ...
                    if (role.position >= botMember.roles.highest.position) {
                        await i.editReply(`I created some roles, but ${role} is above my highest role. Move Nymera's role above every level role, then run this command again.`);
                        return;
                    }
