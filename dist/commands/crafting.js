...
export const craftingCommands = [
    ...
    },
    {
        ...
        async execute(i) {
            await seedGuildEconomy(i.guildId);
            const key = i.options.getString("recipe", true).toLowerCase();
            const recipe = recipes[key];
            if (!recipe)
                return void await i.reply({ content: "Recipe not found. Use `/recipes`.", ephemeral: true });
                ...
                await prisma.$transaction(async (tx) => {
                    ...
                    for (const [itemKey, needed] of Object.entries(recipe.ingredients)) {
                        ...
                        if (!owned || owned.quantity < amount)
                            throw new Error(`MISSING:${itemKey}:${amount}`);
                        if (owned.quantity === amount)
                            await tx.inventoryItem.delete({ where: { id: owned.id } });
                        else
                            await tx.inventoryItem.update({ where: { id: owned.id }, data: { quantity: { decrement: amount } } });
                    }
    ...
    },
    {
        ...
        async execute(i) {
            ...
            if (sub === "select" || sub === "rename") {
                ...
                if (!familiar)
                    return void await i.reply({ content: "That familiar is not in your collection.", ephemeral: true });
                if (sub === "select") {
            ...
            if (!familiar)
                return void await i.reply({ content: "Select or summon an active familiar first.", ephemeral: true });
            if (sub === "view") {
    ...
    },
    {
        ...
        async execute(i) {
            ...
            if (sub === "offer") {
                ...
                if (recipient.bot || recipient.id === i.user.id)
                ...
                if (!item)
                    return void await i.reply({ content: "Item not found.", ephemeral: true });
                ...
                if (!owned || owned.quantity < quantity)
                    return void await i.reply({ content: "You do not own enough of that item.", ephemeral: true });
            ...
            if (!trade || trade.expiresAt <= new Date())
                return void await i.reply({ content: "That trade is unavailable or expired.", ephemeral: true });
            if (sub === "decline") {
            ...
            try {
                await prisma.$transaction(async (tx) => {
                    ...
                    if (buyer.wallet < trade.requestedMarks)
                        throw new Error("FUNDS");
                    if (sellerItem.quantity === trade.quantity)
                        await tx.inventoryItem.delete({ where: { id: sellerItem.id } });
                    else
                        await tx.inventoryItem.update({ where: { id: sellerItem.id }, data: { quantity: { decrement: trade.quantity } } });
                    await tx.inventoryItem.upsert({
