...
export const utilityExpansionCommands = [
    {
    ...
    },
    {
        data: new SlashCommandBuilder().setName("ritual-reminder").setDescription("Schedule a gentle reflective-practice reminder")
            ...
            .addChoices({ name: "Grounding pause", value: "Take a grounding pause: notice five things you see, four you feel, and one slow breath." }, { name: "Moon journal", value: "Open your moon journal and write one honest sentence about what feels visible today." }, { name: "Boundary check", value: "Review one boundary: name what you need and the action that remains within your control." }, { name: "Candle focus", value: "Begin a short focus ritual with a safely placed candle or battery candle and a realistic intention." }))
            .addIntegerOption(o => o.setName("minutes").setDescription("Minutes from now").setRequired(true).setMinValue(1).setMaxValue(43_200)),
        async execute(i) {
            const minutes = i.options.getInteger("minutes", true);
            const dueAt = new Date(Date.now() + minutes * 60_000);
    ...
    },
    {
        ...
        async execute(i) {
            const firstKey = i.options.getString(i.options.getSubcommand() === "sign" ? "sign" : "first", true);
            ...
            const secondKey = i.options.getString("second", true);
        ...
        async execute(i) {
            if (i.options.getSubcommand() === "member") {
                ...
                await i.reply({ embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle(`${user.username}'s Nymera Statistics`)
                            .addFields({ name: "Progress", value: `Prestige ${account.prestige} • Level ${account.level}\n${account.xp.toLocaleString()} XP`, inline: true }, { name: "Economy", value: `${(account.wallet + account.bank).toLocaleString()} ${currency}\nBank tier ${account.bankLevel + 1}`, inline: true }, { name: "Activity", value: `${account.messages.toLocaleString()} messages\n${account.crafted} crafted`, inline: true }, { name: "Games", value: `${games._sum.won ?? 0}/${games._sum.played ?? 0} wins\n${games._sum.earned ?? 0} net ${currency}`, inline: true }, { name: "Collection", value: `${items} unique items\n${familiars} familiars`, inline: true }, { name: "Achievements", value: String(achievements), inline: true })] });
                return;
            }
            ...
            await i.reply({ embeds: [new EmbedBuilder().setColor(0x6f42c1).setTitle(`${i.guild.name} • Nymera Statistics`)
                        .addFields({ name: "Economy", value: `${accounts} accounts\n${((totals._sum.wallet ?? 0) + (totals._sum.bank ?? 0)).toLocaleString()} ${currency}`, inline: true }, { name: "Community activity", value: `${totals._sum.messages ?? 0} eligible messages\n${totals._sum.crafted ?? 0} crafted`, inline: true }, { name: "Games", value: `${games._sum.played ?? 0} played\n${games._sum.won ?? 0} wins`, inline: true }, { name: "Collections", value: `${familiars} familiars`, inline: true }, { name: "Trading", value: `${trades} completed trades`, inline: true }, { name: "Giveaways", value: `${giveaways} completed`, inline: true })] });
        }
    },
    {
