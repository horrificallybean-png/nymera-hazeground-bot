...
async function rewardMarks(guildId, userId, amount, type, note) {
...
async function grantItem(guildId, userId, key) {
...
export const eventCommands = [
    ...
    },
    {
        ...
        async execute(i) {
            if (!isHalloweenSeason())
                return void await i.reply({ content: "The Halloween Haunting awakens each October.", ephemeral: true });
            const period = dayKey();
            const key = { guildId_userId_eventKey_periodKey: { guildId: i.guildId, userId: i.user.id, eventKey: "halloween", periodKey: period } };
            if (await prisma.eventProgress.findUnique({ where: key }))
                return void await i.reply({ content: "You already knocked on the crypt door today.", ephemeral: true });
            const roll = secureInt(1, 100);
            let detail;
            if (roll <= 8)
                detail = `Rare treat: **${await grantItem(i.guildId, i.user.id, "halloween_badge")}**!`;
            else if (roll <= 25)
                detail = `Mysterious treat: **${await grantItem(i.guildId, i.user.id, "haunted_lootbox")}**!`;
            else {
    ...
    },
    {
        ...
        async execute(i) {
            if (!fullMoonActive())
                return void await i.reply({ content: `The offering circle opens during the Full Moon. Current phase: ${moonPhase().name}.`, ephemeral: true });
            const period = monthKey();
            const key = { guildId_userId_eventKey_periodKey: { guildId: i.guildId, userId: i.user.id, eventKey: "full_moon", periodKey: period } };
            if (await prisma.eventProgress.findUnique({ where: key }))
                return void await i.reply({ content: "You already joined this full-moon gathering.", ephemeral: true });
            const amount = 750;
    ...
    },
    {
        data: new SlashCommandBuilder().setName("dbd-build").setDescription("Receive a Dead by Daylight perk-build suggestion")
            ...
            .addChoices({ name: "Chase", value: "chase" }, { name: "Rescue / Control", value: "support" }, { name: "Stealth / Tracking", value: "information" }, { name: "Objectives / Slowdown", value: "objectives" })),
        async execute(i) {
    ...
    },
    {
        ...
        async execute(i) {
            ...
            if (saved?.claimedAt)
                return void await i.reply({ content: "You already completed today's DBD challenge.", ephemeral: true });
            const question = dbdTrivia[secureInt(0, dbdTrivia.length - 1)];
            const row = new ActionRowBuilder().addComponents(question.choices.map((choice, index) => new ButtonBuilder().setCustomId(`dbdevent:${i.id}:${index}`).setLabel(choice).setStyle(ButtonStyle.Secondary)));
