import { ChannelType, PermissionFlagsBits, SlashCommandBuilder, userMention } from "discord.js";
import { ensureGuild, prisma } from "../database.js";
const reasonOption = (o) => o.setName("reason").setDescription("Reason").setMaxLength(500);
const target = (builder) => builder.addUserOption(o => o.setName("user").setDescription("Member").setRequired(true));
export const moderationCommands = [
    ...
    },
    {
        ...
        async execute(i) {
            if (!i.channel || i.channel.type !== ChannelType.GuildText)
                return void await i.reply({ content: "Use this in a text channel.", ephemeral: true });
    ...
    },
    {
        ...
        async execute(i) {
            if (!i.channel || !("setRateLimitPerUser" in i.channel))
                return void await i.reply({ content: "Unsupported channel.", ephemeral: true });
            await i.channel.setRateLimitPerUser(i.options.getInteger("seconds", true), `By ${i.user.tag}`);
            await i.reply("Slowmode updated.");
        }
    },
    ...["lock", "unlock"].map((name) => ({
